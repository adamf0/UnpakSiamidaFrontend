...
const TemplateRenstraFormPage = () => {
  ...
  const buildRowLabel = (gd, target) =>
    `${gd.Type.toUpperCase()} - ${gd.Jenjang.toUpperCase()} - ${target.Nama.toUpperCase()}`;
  ...
  const fetchTemplateRenstraEdit = async (indikatorUuid) => {
    ...
    setTimeout(async () => {
      try {
        ...
        if(!res.ok){
          setErr("template", data.message || "Gagal memuat data");
          return;
        }
        ...
        data.forEach((item) => {
          ...
          trigger([
            //ini sidah masuk namun tidak muncul di ui
            `rows.${rowKey}.klasifikasi`,
            `rows.${rowKey}.satuan`,
            `rows.${rowKey}.pembagian_tugas`,
            ...(watch("indikator_renstra")?.TipeTarget === "range"
  ...
  useEffect(() => {
    ...
    const selected = indikatorOptions.find((i) => i.Uuid === uuidIndikator);
  ...
  const onSubmit = async (data) => {
    ...
    const totalSendData = Object.values(data.rows).filter(
      (row) => row?.enable
    ).length;
    ...
    const payloads = Object.entries(data.rows)
      ...
      .map(([key, row]) => {
        const [type, jenjang, nama] = key
          .split("-")
          .map((v) => v.trim().toLowerCase());
        const targetAudit = fakultasUnits.find(
          (item) =>
            item.Type.toLowerCase() === type &&
            item.Jenjang.toLowerCase() === jenjang &&
            item.Nama.toLowerCase() === nama
        );
        const kategori = !isEmpty(targetAudit?.Type.toLowerCase())
          ? `${targetAudit?.Type.toLowerCase()}#${
              targetAudit?.Type.toLowerCase() === "fakultas"
                ? "all"
                : targetAudit?.Jenjang.toLowerCase()
            }`
          : "";
            ...
            <div className="mb-6 p-6 space-y-4 border rounded-lg w-full">
              ...
              {loading.indikator ? (
                ...
                <SearchSelect
                  ...
                  onChange={(item) =>
                    setValue("indikator_renstra", item, {
                      shouldValidate: true,
                    })
                  }
                />
...
export const GroupDataTable = ({
  ...
  return (
    <div
      key={gd.Type + gd.Jenjang}
      className="p-6 border rounded-lg w-full mt-2"
    >
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-sm font-medium">
          {gd.Jenjang
            ? `${gd.Type.toUpperCase()} - ${gd.Jenjang.toUpperCase()}`
            : gd.Type.toUpperCase()}
        </h2>
        <button
          type="button"
          ...
          <tbody>
            {watch("indikator_renstra") &&
              gd.Targets.map((target) => {
                ...
                const errorKlasifikasiMessage = get(
                  errors,
                  fieldKlasifikasi
                )?.message;
                const errorPembagianTugasMessage = get(
                  errors,
                  fieldPembagianTugas
                )?.message;
                ...
                if (tipeTarget === "range") {
                  ...
                  targetInput = (
                    <div className="flex flex-wrap justify-center gap-2">
                      <TextInput
                        ...
                        onChange={(e) => {
                          setValue(fieldTargetMin, e.target.value, {
                            shouldValidate: true,
                          });
                          markRowActive(rowKey);
                        }}
                      ...
                      <TextInput
                        ...
                        onChange={(e) => {
                          setValue(fieldTargetMax, e.target.value, {
                            shouldValidate: true,
                          });
                          markRowActive(rowKey);
                        }}
                  ...
                  targetInput = (
                    <TextInput
                      ...
                      onChange={(e) => {
                        setValue(fieldTarget, e.target.value, {
                          shouldValidate: true,
                        });
                        markRowActive(rowKey);
                      }}
                  ...
                  >
                    ...
                    <td className="px-3 py-3">
                      <div className="flex flex-wrap gap-4 justify-center">
                        <RadioButton
                          ...
                          onChange={() => {
                            setValue(fieldKlasifikasi, "minor", {
                              shouldValidate: true,
                            });
                            markRowActive(rowKey);
                          }}
                        ...
                        <RadioButton
                          ...
                          onChange={() => {
                            setValue(fieldKlasifikasi, "major", {
                              shouldValidate: true,
                            });
                            markRowActive(rowKey);
                          }}
                    ...
                    <td className="px-3 py-3">
                      <TextInput
                        ...
                        onChange={(e) => {
                          setValue(fieldSatuan, e.target.value, {
                            shouldValidate: true,
                          });
                          markRowActive(rowKey);
                        }}
                    ...
                    <td className="px-3 py-3">
                      <div className="flex flex-wrap gap-4 justify-center">
                        <RadioButton
                          ...
                          onChange={() => {
                            setValue(fieldPembagianTugas, "auditor1", {
                              shouldValidate: true,
                            });
                            markRowActive(rowKey);
                          }}
                        ...
                        <RadioButton
                          ...
                          onChange={() => {
                            setValue(fieldPembagianTugas, "auditor2", {
                              shouldValidate: true,
                            });
                            markRowActive(rowKey);
                          }}
